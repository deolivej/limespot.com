...
<section id="k2Container" class="userView<?php if($this->params->get('pageclass_sfx')) echo ' '.$this->params->get('pageclass_sfx'); ?>">
	...
	<section class="itemList">
		...
		<article class="itemView<?php if(!$item->published || ($item->publish_up != $this->nullDate && $item->publish_up > $this->now) || ($item->publish_down != $this->nullDate && $item->publish_down < $this->now)) echo ' itemViewUnpublished'; ?><?php echo ($item->featured) ? ' itemIsFeatured' : ''; ?> clearfix"> <?php echo $item->event->BeforeDisplay; ?> <?php echo $item->event->K2BeforeDisplay; ?>
			...
			if(
				$item->params->get('userItemHits') ||
				$item->params->get('userItemCategory') ||
				$item->params->get('itemPrintButton') ||
				$item->params->get('itemEmailButton') ||
				($item->params->get('userItemAuthor') && empty($item->created_by_alias)) ||
				($item->params->get('userItemTags') && count($item->tags))
			) :
			?> containsItemInfo<?php endif; ?>"> 	
				...
				<header>
					<?php 
						if(
							...
							($this->params->get('userItemCommentsAnchor') && ( ($this->params->get('comments') == '2' && !$this->user->guest) || ($this->params->get('comments') == '1')))
						): 
					?>	
					<ul>
				...
				<a class="itemReadMore button" href="<?php echo $item->link; ?>"> 
			...
			if(
				$item->params->get('userItemHits') ||
				$item->params->get('userItemCategory') ||
				($item->params->get('userName') && empty($item->created_by_alias)) ||
				($item->params->get('userItemTags') && count($item->tags))
			) :
			?>
			<aside class="itemAsideInfo">
				<?php if($item->params->get('userName') && empty($item->created_by_alias)): ?>
					<?php if($item->params->get('userImage') && !empty($item->author->avatar)):?>
					<img src="<?php echo $item->author->avatar; ?>" alt="<?php echo K2HelperUtilities::cleanHtml($item->author->name); ?>" />
					<?php endif; ?>
					
					<?php if($item->params->get('userEmail')):?>
					<?php echo JHTML::_('Email.cloak', $item->author->email); ?>
					<?php endif; ?>
				...
				<ul>
					...
					<li class="itemCategory">
						...
						<a href="<?php echo $item->category->link; ?>"><?php echo $item->category->name; ?></a> </li>
					<?php endif; ?>
					
					<?php if($item->params->get('userItemHits')): ?>
					<li class="itemHits"> <?php echo JText::_('K2_READ'); ?> <?php echo $item->hits; ?> <?php echo JText::_('K2_TIMES'); ?> </li>
					<?php endif; ?>
